...
<body>
	<c:if test="${board.WRITE_TYPE=='비공개'}">
		<c:if
			...
			<div id="detail">
				...
				${B_CONTENT}
				<hr size="1" width="100%">
				<c:if test="${sessionScope.email==board.WRITER}">
				...
				<input type="button" value="목록" class="inputb"
					onclick="location.href='boardList.puzzle?DEPT_NUM=${board.DEPT_NUM}'">
			</div>
			<div>
				<div align="left">
				...
				<form:form enctype="multipart/form-data" action="writeComment.do"
					...
					<input type="hidden" name="CNO" value="${cnum}" />
					<div id="comment_div">
							...
							<c:forEach var="article" items="${commentList}">
								<tr>
									...
									<td><fmt:formatDate value="${article.REG_DATE}"
											pattern="yy/MM/dd" /></td>
									<td><c:if test="${sessionScope.email==article.EMAIL}">
											<input type="button" value="삭제" class="inputb"
	...
	<c:if test="${board.WRITE_TYPE=='공개'}">
		...
		<div>
			...
			<hr size="1" align="left" width="50%">
			<form method="POST" action="writeComment.puzzle"
				commandName="command">
				<input type="hidden" name="BNO" value="${board.BNO}" /> 
				<input type="hidden" name="EMAIL" value="${sessionScope.email}" />
				<input type="hidden" name="CNO" value="${cnum}" />
				<div id="comment_div">
					<textarea rows="4" cols="80" name="CONTENT" id="comment_subject"></textarea>
