...
// 			type:"post"		// 	포스트방식
// 	 			,url: "deptermentList.puzzle"		// 	url 주소	 			
// // 	 			,data: dept_Type//  요청에 전달되는 프로퍼티를 가진 객체
// 	 			,dataType:"json"
// 	 			,success:function(args){	//응답이 성공 상태 코드를 반환하면 호출되는 함수
// 	 				$("#category").append('<li>&nbsp;'+dept_Type+'</li>');
// 	 				$("#add").on('click',function(){
...
/* 	function category(){
		#("#category")>li.on('click',function(){
			#(this).appendTo('<button onclick='edit()'>수정</button><button onclick='cancel()'>취소</button>');
		}
	} */
	function edit() {
		var dept_Type = $(this).val();
		$("#editform").after("<input type=button onclick='editOk()' value='수정'/><input type=button onclick='edit_cancle()' value='취소'/>");
...
// 			,data:params	//  요청에 전달되는 프로퍼티를 가진 객체
	...
	$(function(){	
		...
		$("#category>li").one("click",function(){
			...
			$(this).append("<input type=text onclick='edit()' value='"+dept_Type+"'/><input type=button id='editform' onclick='edit()' value='수정'/><input type=button onclick='edit_cancle()' value='취소'/>");
		});
	});
